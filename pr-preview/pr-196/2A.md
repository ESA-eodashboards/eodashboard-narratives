...
1. **Data selection and temporal compositing:** the study focuses on the Milan and Monza-Brianza area during summer 2021 (June–August). Sentinel-2 Level-2A images were selected based on acquisition date and cloud cover. Six surface indicators (NDVI, NDRE, NDBI, BSI, MNDWI, Albedo) were then derived from the selected observations. For each indicator, a pixel-wise temporal median was calculated using valid pixels to represent typical summer surface conditions. Tree Cover Density and Imperviousness Density were taken from the corresponding 2021 annual products and therefore did not require temporal compositing, while LST was represented by the corresponding summer composite.
